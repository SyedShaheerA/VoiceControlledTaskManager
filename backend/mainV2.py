...
async def chat_endpoint(
    ...
    if session["last_task_id"] is not None:
        ...
        last_task_hint = (
            f"\n\n*** CRITICAL CONTEXT ***"
            f"\nThe LAST task the user explicitly referenced was: '{ltitle}' (ID: {lid})."
            f"\nIf the user says ANYTHING vague — 'the previous one', 'that one', 'it',"
            f" 'actually', 'change that', 'change it', 'move it' — you MUST use target_task_id: {lid}."
            f"\nDo NOT pick a different task ID unless the user explicitly names a different task by its title."
    ...
    try:
        ...
        if intent == "CREATE":
            ...
            if tid:
                ...
                # Look up the task title so the next hint is human-readable
